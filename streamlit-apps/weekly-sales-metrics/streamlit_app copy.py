...
WEEK_CUTOFF_SQL = "DATE_TRUNC('WEEK', CURRENT_DATE())"  # = Monday of current week
...
def load_filter_options():
    ...
    recent_filter = "deal_created_date >= DATEADD('MONTH', -13, CURRENT_DATE())"
    ...
    """)
    types = _vals(f"SELECT DISTINCT deal_type FROM {DWH}.FACT_DEALS WHERE deal_type IS NOT NULL AND deal_created_date >= DATEADD('MONTH', -13, CURRENT_DATE())")
    ...
    """)
    nna_df = conn.query(f"SELECT MIN(deal_net_new_arr) AS mn, MAX(deal_net_new_arr) AS mx FROM {DWH}.FACT_DEALS WHERE deal_net_new_arr IS NOT NULL AND deal_created_date >= DATEADD('MONTH', -13, CURRENT_DATE())")
    mn = float(nna_df.iloc[0]["MN"] or 0)
    mx = float(nna_df.iloc[0]["MX"] or 1_000_000)
    return teams, owners, types, stages, segs, mn, mx


teams_all, owners_all, types_all, stages_all, segs_all, min_nna, max_nna = load_filter_options()
...
cohort_breakdown = st.sidebar.selectbox("Split by", ["None", "Segment", "HubSpot Team", "Won/Lost"], key="cohort_breakdown")
...
kpi_sql = f"""
SELECT
    COUNT(DISTINCT sk_company) AS num_customers,
...
WITH weeks AS (
    SELECT
        DATEADD('WEEK', -1, DATE_TRUNC('WEEK', CURRENT_DATE())) AS last_full_week_start,
        DATE_TRUNC('WEEK', CURRENT_DATE()) AS last_full_week_end,
        DATEADD('WEEK', -2, DATE_TRUNC('WEEK', CURRENT_DATE())) AS prior_week_start
)
SELECT
...
with cohort_col3:
    cohort_metric = st.radio("Metric", ["$ (ARR)", "# (Count)"], horizontal=True, key="cohort_metric")
with cohort_col4:
...
def load_cohort_data(from_col, to_col, to_cond, trunc, metric, global_filter_sql):
    ...
    WHERE {from_col} IS NOT NULL
      ...
      AND {from_col} >= DATEADD('MONTH', -13, CURRENT_DATE())
      AND {from_col} < DATE_TRUNC('WEEK', CURRENT_DATE())
      {global_filter_sql}
    """
    ...
    WHERE {from_col} IS NOT NULL
      ...
      AND {to_cond}
      AND {to_col} < DATE_TRUNC('WEEK', CURRENT_DATE())
...
if df_cohort_base.empty:
    ...
    BREAKDOWN_COL_MAP = {"Segment": "SEGMENT", "HubSpot Team": "TEAM"}
...
with pri_row1_col1:
    ...
    pacing_gran = st.radio("Granularity", ["Weekly", "Monthly", "Quarterly"], horizontal=True, key="pacing_gran")
    ...
    st.caption(f"Data up to {_pacing_cutoff_date.strftime('%B %d, %Y')} (end of last full week)")
    ...
    if pacing_view.startswith("Qualified Pipeline"):
    ...
    if pacing_view == "Qualified Pipeline (#)":
    ...
    WHERE fd.deal_net_new_arr > 0
      ...
      {extra_where}
      AND {_date_col} >= DATEADD('{trunc_unit}', -{lookback_periods}, DATE_TRUNC('{trunc_unit}', CURRENT_DATE()))
    ...
    if not df_pacing_cutoff.empty:
        ...
        if pacing_gran == "Weekly":
            ...
            _end = pd.to_datetime(df_pacing_cutoff["PERIOD_LABEL"].max())
            ...
            _end = pd.to_datetime(df_pacing_cutoff["PERIOD_LABEL"].max() + "-01")
        ...
        if _has_breakdown:
            ...
            st.altair_chart(pacing_chart, use_container_width=True)
        else:
            # Single line chart
            pacing_chart = (
                alt.Chart(df_pacing_cutoff)
...
with pri_row1_col2:
    ...
    st.caption("Target: no more than 15% of open deals with missing fields")
...
with pri_row2_col1:
    ...
    WITH weekly_metrics AS (
        ...
        WHERE fd.is_closed = TRUE
          ...
          AND fd.deal_type = 'newbusiness'
          AND fd.deal_closed_date >= DATEADD('QUARTER', -3, DATE_TRUNC('QUARTER', CURRENT_DATE()))
          AND fd.deal_closed_date < DATE_TRUNC('WEEK', CURRENT_DATE())
          {build_global_filters_sql(has_employee_join=True)}
...
with pri_row2_col2:
    ...
    WITH deals AS (
        ...
        LEFT JOIN {DWH}.DIM_EMPLOYEE de
            ON fd.sk_sales_owner = de.sk_employee
        WHERE fd.deal_created_date >= DATEADD('{ph_trunc}', -{ph_periods}, DATE_TRUNC('{ph_trunc}', CURRENT_DATE()))
          AND fd.deal_created_date < DATE_TRUNC('WEEK', CURRENT_DATE())
    ...
    SELECT
        ...
        COUNT(CASE
            WHEN segment = 'Mid-Market' AND (
                (qualified_date IS NULL AND DATEDIFF('DAY', deal_created_date, DATE_TRUNC('WEEK', CURRENT_DATE())) > 30)
                OR (qualified_date IS NOT NULL AND DATEDIFF('DAY', deal_created_date, qualified_date) > 30)
            ) THEN 1
            WHEN segment = 'Enterprise' AND (
                (qualified_date IS NULL AND DATEDIFF('DAY', deal_created_date, DATE_TRUNC('WEEK', CURRENT_DATE())) > 45)
    ...
    GROUP BY create_period, segment, hubspot_team
        ...
        with st.expander("Show raw deal data", expanded=False):
            ...
            SELECT
                ...
                CASE
                    ...
                    ELSE DATEDIFF('DAY', fd.deal_created_date, DATE_TRUNC('WEEK', CURRENT_DATE()))
