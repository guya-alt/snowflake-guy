...
-- age and ramp-up are NULL. Scoped to employees who currently own at least one
-- account (CS or AM) in DIM_COMPANY.
SELECT
...
JOIN (
    SELECT SK_CSM_OWNER AS SK_EMPLOYEE, 'CS' AS ROLE
    FROM PORT_ANALYTICS_PROD.DWH.DIM_COMPANY WHERE SK_CSM_OWNER IS NOT NULL
    UNION ALL
    SELECT SK_ACCOUNT_OWNER, 'AM'
    FROM PORT_ANALYTICS_PROD.DWH.DIM_COMPANY WHERE SK_ACCOUNT_OWNER IS NOT NULL
) r ON r.SK_EMPLOYEE = e.SK_EMPLOYEE
GROUP BY e.DISPLAY_NAME, e.ORIGINAL_START_DATE
ORDER BY OWNER;
